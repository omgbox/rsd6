...
<body>
    <div class="wrapper">
        ...
        <main class="container">
            ...
            <div id="player-section"><div class="card"><div id="loading-message"><h2>Preparing stream...</h2><p id="loading-details">Connecting to source...</p></div><div id="video-container" style="display:none;"><video id="video" controls autoplay preload="auto"></video></div></div><a href="#" id="stream-another" role="button" class="button">Stream Another Video</a></div>
        </main>
    </div>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          let hls=null,pollingInterval=null,sessionId=null,heartbeatInterval=null;
          ...
          function getSessionId(){let e=localStorage.getItem('userSessionId');if(e)return e;let t=generateSessionId();localStorage.setItem('userSessionId',t);return t}sessionId=getSessionId();
          ...
          function showFormView(){if(hls)hls.destroy();if(pollingInterval)clearInterval(pollingInterval);stopHeartbeat();video.pause();video.src="";fetch(`/stop/${sessionId}`,{method:'POST'}).finally(()=>{playerSection.style.display='none';streamForm.style.display='block';loadingMessage.style.display='block';videoContainer.style.display='none';loadingDetails.textContent='Connecting to source...'})}
          function showPlayerView(){streamForm.style.display='none';playerSection.style.display='block';startPollingForPlaylist();startHeartbeat()}
          ...
          streamForm.addEventListener('submit',function(e){e.preventDefault();const u=urlInput.value.trim();if(!u)return;fetch(`/stream/${sessionId}`,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`url=${encodeURIComponent(u)}`}).then(r=>{if(r.ok){showPlayerView()}else if(r.status===429){return r.json().then(e=>alert(e.error))}else{alert('Error starting stream.')}});urlInput.value=''});
...
def start_stream_process(session_id, video_url):
    ...
    with streams_lock: active_streams[session_id] = {'process': process, 'last_seen': time.time(), 'process_running': True}
...
async def handle_stream_post(request):
    ...
    with streams_lock:
        running_streams = sum(1 for s in active_streams.values() if s.get('process') and s['process'].poll() is None)
        if running_streams >= MAX_CONCURRENT_STREAMS:
            logging.warning(f"Rejecting new stream request. {running_streams}/{MAX_CONCURRENT_STREAMS} streams already running.")
            return web.json_response({'error': "Server is at maximum capacity. Please try again in a moment."}, status=429)
    data = await request.post(); video_url = data.get('url', '').strip()
    if not video_url: raise web.HTTPBadRequest(reason="URL is missing")
    processed_url = f"http://rsd.ovh/stream?url={video_url}" if video_url.startswith("magnet:?") else video_url
...
async def start_background_tasks(app): app['reaper_task'] = asyncio.create_task(reaper_task(app))
...
def main():
    # REFACTOR: Added check for tmpfs to guide user toward optimal setup.
